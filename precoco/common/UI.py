...
from precoco.common.miscfunctions import install_update
...
def create_ui(tree, version, file_path, up_to_date):
    ...
    if not up_to_date:
        ButtonUpdate = Button(FrameTopMaster,
                              text='Click here to update!',
                              width=30,
                              font=('Arial', 9, 'bold'),
                              fg='red',
                              command=install_update)
        ButtonUpdate.pack(side='right')
    else:
        ChangelogButton = Button(FrameTopMaster,
    ...
    FrameNumbers = VerticalScrolledFrame(tk,
                                         width=150,
                                         height=50,
                                         borderwidth=2,
                                         relief="groove")
    ...
    CheckboxFalseTextBreaks = Checkbutton(FrameChecks, anchor='w', text='remove false text breaks',
    ...
    if gf.flag_is_formatted:
        CheckboxSpanHeaders = Checkbutton(FrameChecks, anchor='w', text='analyze heading (BETA)',
    ...
    CheckboxSetUnorderedLists = Checkbutton(FrameChecks, anchor='w', text='set unordered lists',
                                            variable=gf.b_set_unordered_lists)
    CheckboxIndentUnorderedLists = Checkbutton(FrameChecks, anchor='w', text='Indent unordered lists?',
    ...
    if gf.b_fonds_report.get():
        ...
        CheckboxTsdFix = Checkbutton(FrameChecks, anchor='w', text='fix tsd separators',
                                     variable=gf.b_fix_tsd_separators)
        CheckboxBreakFondsTable = Checkbutton(FrameChecks, anchor='w', text='break Vermögensaufstellung',
                                              variable=gf.b_break_fonds_table)
        CheckboxTsdFix.pack(side='top', anchor='w')
        CheckboxBreakFondsTable.pack(side='top', anchor='w')

    arg_list = [tree, false_number, false_words, ListboxNumbers.return_list(), ListboxWords.return_list(), file_path,
                tk]
